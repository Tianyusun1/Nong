...
@app.route('/')
def index():
    """商城首页：已接入核心推荐算法 + 真实销量热榜"""
    q = request.args.get('q', '')

    # 1. 基础查询构造器 (所有上架商品)，预加载 SKU 以优化性能
    base_query = Product.query.filter(Product.is_on_sale == True).options(joinedload(Product.skus))

    # ==========================================
    # 🔥 核心新增：获取销量最高的热门商品 (Top 4)
    # ==========================================
    popular_products = []
    try:
        # 逻辑：连接商品和订单详情表 -> 按商品ID分组 -> 统计销售总数量 -> 倒序 -> 取前4名
        # 注意：使用 func.sum(OrderItem.quantity) 比 count 更能反映真实热度
    ...
    # 💡 兜底策略：如果还没有任何订单数据，或者数据库查询报错
    # 则默认取最新的 4 个已上架商品，防止前端“热门搜索”区域留白
    if not popular_products:
        popular_products = base_query.order_by(Product.product_id.desc()).limit(4).all()

    # ==========================================
    # 2. 处理主列表显示逻辑 (搜索 or 个性化推荐)
    # ==========================================
    if q:
        # --- 搜索模式 ---
        ...
        # 如果用户已登录，尝试加载 CF 个性化推荐
        if 'user_id' in session:
            try:
                # 必须在 app context 下执行 recommender
                with app.app_context():
                ...
                # 算法出错时不中断页面，仅在后台打印日志
                print(f"⚠️ 推荐算法调用失败，已回退到默认列表: {e}")

    # 3. 格式化主列表数据 (计算最低价 min_price，供前端卡片展示)
    final_products_data = []
    for product in products:
        # 容错处理：如果没有 SKU，价格显示 0.00
        min_price = min(sku.price for sku in product.skus) if product.skus else Decimal('0.00')
        final_products_data.append({
            'product': product,
            'min_price': min_price
